---
title: "グループ（ロール）を一括で登録、編集する"
weight: 600
---

グループ（ロール）のデータを、CSVファイルを使って、一括で登録したり更新したりできます。  
{{% enabled JP %}}
グループ（ロール）は、サイボウズ Officeとメールワイズには反映されません。
{{% /enabled %}}
...
## STEP1 サンプルデータを{{%service%}}に登録する {#list_csv_group_10}

サンプルデータは1つあれば十分です。  

1. サンプルとして、グループ（ロール）を登録します。  
  手順は、[グループ（ロール）を追加する](/general/ja/admin/list_useradmin/list_group/set_group.html)を参照してください。

{{% note %}}
グループ（ロール）を登録済みの場合は、このSTEP1は不要です。STEP2に進みます。
{{% /note %}}

## STEP2 登録済みのデータをファイルに書き出す {#list_csv_group_20}

1. 「{{%slash%}}共通管理」画面の「ユーザー管理」の[ファイルへの書き出し]をクリックします。
  {{< screen src="/general/img-ja/group_img01.png" alt="ファイルへの書き出しが赤枠で囲まれた画像">}}

1. 「ファイルに書き出す項目」に「グループ（ロール）」を選択します。  
...
1. [書き出す]をクリックします。  
  ...
  {{< screen src="/general/img-ja/group_img02.png" alt="[書き出す]が赤枠で囲まれた画像">}}

1. Webブラウザーのファイル保存機能を使って、ファイルを保存します。  

## STEP3 CSVファイルを更新する {#list_csv_group_30}

STEP2で書き出したCSVファイルに、データを追記したり、修正したりします。  

1. 書き出したCSVファイルをExcelで開きます。  

1. [グループ（ロール）のファイルフォーマット](/general/ja/admin/list_useradmin/list_csv/list_format/group.html) を確認し、CSVファイルのデータを更新します。  

1. CSVファイルを保存します。  
  ファイル名を変更して保存する場合は、ファイルの種類に「CSV（カンマ区切り）（&ast;.csv）」を選択します。

## STEP4 更新したCSVファイルを読み込む {#list_csv_group_40}

1. 「{{%slash%}}共通管理」画面の「ユーザー管理」の[ファイルからの読み込み]をクリックします。  

1. 「ファイルから読み込む項目」で、「グループ（ロール）」を選択します。  
...
1. 「プレビュー」で、データの誤りなどがないか、確認します。  
  「プレビュー」には、ファイルの先頭から5行分のデータが表示されます。  
  {{< screen src="/general/img-ja/group_img03.png" alt="ファイルからの読み込み画面">}}
